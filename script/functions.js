...
function drawDotAndAngle(figureNumber, dotNumber) {
  ...
  // Dibujado de línea
  if (dotNumber > 0)
    drawLine(dots[figureNumber][dotNumber - 1], dots[figureNumber][dotNumber]);
  // Dibujado de ángulo
  if (dotNumber > 1) {
...
function closeFigure(figureAmount, dotAmount) {
  if (dotAmount > 0) {
    drawLine(dots[figureAmount][dotAmount - 1], dots[figureAmount][dotAmount]);
  }
  context.closePath();
...
function onCanvasClick(event) {
  ...
  // Caso 1: el nuevo punto está cerca del primero
    ...
    dots[lastFigure].push(dots[lastFigure][0]);
...
function loadCanvas() {
  ...
  for (let i = 0; i <= lastFigure; i++) {
    if (dots[i].length != 0) {
      ...
      if (
        dist(dots[i][lastFigureDotCounter], firstDot) < minimumDistance &&
        lastFigureDotCounter != 0
      ) {
